...
public interface LocalityToLocalityTravelInfoRepository
	...
	@Query(value = "SELECT new com.excentria_it.wamya.domain.JourneyTravelInfo(l.distance, l.hours, l.minutes) FROM LocalityToLocalityTravelInfoJpaEntity l JOIN l.localityOne l1 JOIN l.localityTwo l2 WHERE (l1.id = ?1 AND l2.id = ?2) OR (l1.id = ?2 AND l2.id = ?1)")
