...
public interface GeoPlaceToGeoPlaceTravelInfoRepository
	...
	@Query(value = "SELECT new com.excentria_it.wamya.domain.JourneyTravelInfo(g.distance, g.hours, g.minutes) FROM GeoPlaceToGeoPlaceTravelInfoJpaEntity g JOIN g.geoPlaceOne p1 JOIN g.geoPlaceTwo p2 WHERE (p1.id = ?1 AND p2.id = ?2) OR (p1.id = ?2 AND p2.id = ?1)")
