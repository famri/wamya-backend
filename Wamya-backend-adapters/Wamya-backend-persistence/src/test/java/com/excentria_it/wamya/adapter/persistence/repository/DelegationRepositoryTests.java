...
public class DelegationRepositoryTests {
	...
	void testFindByCountry_IdAndNameLikeIgnoringCase() {
		...
		List<AutoCompleteDelegationDto> delegationsDtosResult = delegationRepository
				.findByCountry_IdAndNameLikeIgnoringCase(country.getId(),
						delegations.get(0).get(0).getPossibleNames().substring(0, 4), "fr_FR",
						firstPageWithFiveElements);
		// then
		assertEquals(delegations.get(0).get(0).getId(), delegationsDtosResult.get(0).getId());
		assertEquals(delegations.get(0).get(0).getName("fr_FR"), delegationsDtosResult.get(0).getName());
		assertThat(delegationsDtosResult.size()).isLessThanOrEqualTo(5);
	}
	...
	void testFindByCountry_IdAndDepartmentNameLikeIgnoringCaseAndNameLikeIgnoringCase() {
		...
		List<AutoCompleteDelegationDto> delegationsDtosResult = delegationRepository
				.findByCountry_IdAndDepartmentNameLikeIgnoringCaseAndNameLikeIgnoringCase(country.getId(),
						delegations.get(0).get(0).getDepartment().getPossibleNames().substring(0, 4),
						delegations.get(0).get(0).getPossibleNames().substring(0, 4), "fr_FR",
						firstPageWithFiveElements);
		// then
		assertEquals(delegations.get(0).get(0).getId(), delegationsDtosResult.get(0).getId());
