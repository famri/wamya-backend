...
public class UserAccountPersistenceAdapterTests {
    ...
    void givenExistentUserAccountByFrenchIccAndFrenchMobileNumberWithoutLeadingZero_whenLoadUserAccountByFrenchMobileNumberSubject_thenReturnTheExistingUserAccount() {
        ...
        given(userAccountRepository.findByMobilePhoneNumber(TestConstants.DEFAULT_INTERNATIONAL_CALLING_CODE,
                TestConstants.DEFAULT_MOBILE_NUMBER.substring(1))).willReturn(userAccountEntityOptional);
    ...
    void givenUserExists_whenExistsByOauthId_thenReturnTrue() {
        ...
        given(userAccountRepository.existsByOauthId(any(String.class))).willReturn(true);

        // when
        Boolean result = userAccountPersistenceAdapter.existsByOauthId(OAuthId.CLIENT1_UUID);
        // then
        assertTrue(result);
    }
    ...
    void givenInexistentUser_whenExistsByOauthId_thenReturnFalse() {
        ...
        given(userAccountRepository.existsByOauthId(any(String.class))).willReturn(false);
    ...
    @Test
    void givenNonExistentUserAccountById_whenUpdateIdentityValidationState_thenThrowUserAccountNotFoundException() {
        // given
        given(userAccountRepository.findById(any(Long.class))).willReturn(Optional.empty());
        // when
        assertThrows(UserAccountNotFoundException.class,
