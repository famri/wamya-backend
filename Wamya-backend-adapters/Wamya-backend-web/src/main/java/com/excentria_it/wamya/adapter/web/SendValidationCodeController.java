package com.excentria_it.wamya.adapter.web;

import javax.validation.Valid;

import org.springframework.http.HttpStatus;
import org.springframework.http.MediaType;
import org.springframework.web.bind.annotation.CrossOrigin;
import org.springframework.web.bind.annotation.ExceptionHandler;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.ResponseStatus;
import org.springframework.web.bind.annotation.RestController;
import org.springframework.web.server.ResponseStatusException;

import com.excentria_it.wamya.application.port.in.SendValidationCodeUseCase;
import com.excentria_it.wamya.application.port.in.SendValidationCodeUseCase.SendValidationCodeCommand;
import com.excentria_it.wamya.application.service.exception.UserAccountNotFoundException;
import com.excentria_it.wamya.common.annotation.WebAdapter;
import com.excentria_it.wamya.domain.ValidationCodeRequest;
import com.excentria_it.wamya.domain.ValidationCodeRequestStatus;

import lombok.RequiredArgsConstructor;

@WebAdapter
@RestController
@RequiredArgsConstructor
@CrossOrigin(origins = "*")
public class SendValidationCodeController {

	private final SendValidationCodeUseCase sendValidationCodeUseCase;

	@PostMapping(path = "/validation-codes/send", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	@ResponseStatus(HttpStatus.ACCEPTED)
	public ValidationCodeRequest sendValidationCode(@Valid @RequestBody SendValidationCodeCommand command) {

		boolean success = sendValidationCodeUseCase.sendValidationCode(command);
		
		ValidationCodeRequest result = new ValidationCodeRequest();

		if (success) {
			result.setStatus(ValidationCodeRequestStatus.REGISTRED);
		} else {
			result.setStatus(ValidationCodeRequestStatus.REJECTED);
		}
		return result;

	}

	@ExceptionHandler({ UserAccountNotFoundException.class })
	public void handleUserAccountNotFoundException(UserAccountNotFoundException exception) {
		throw new ResponseStatusException(HttpStatus.BAD_REQUEST, exception.getMessage(), exception);
	}
}
