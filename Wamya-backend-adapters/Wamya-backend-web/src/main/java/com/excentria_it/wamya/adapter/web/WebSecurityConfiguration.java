	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable().cors().disable().httpBasic().disable().formLogin().disable().sessionManagement()
				.sessionCreationPolicy(SessionCreationPolicy.STATELESS).and().authorizeRequests(authz -> authz
						...
						.antMatchers(HttpMethod.POST, "/accounts/do-request-password-reset/**","/accounts/password-reset/**").permitAll()
						.antMatchers(HttpMethod.GET, "/accounts/password-reset/**").permitAll()
						.antMatchers(HttpMethod.PATCH, "/accounts/me/device-token/**").hasAuthority("SCOPE_profile:write")
						.antMatchers("/users/me/discussions/**").hasAnyAuthority("SCOPE_journey:write", "SCOPE_offer:write")
						...
						.antMatchers(HttpMethod.PATCH, "/journey-requests/{\\d+}/**").hasAuthority("SCOPE_journey:write")
						...
						.antMatchers(HttpMethod.POST, "/validation-codes/sms/validate/**").hasAuthority("SCOPE_profile:write")
